...
|:--------|:------|:-------------|:--------------|
| 1.1.0 | 157 | 2026-07-01 | Minor feature release for the Basic edition. Groups the Simple HTML helpers, TokenMerge/template-mail helpers, save-folder/test-send compatibility work, clearer licensing/setup diagnostics, expanded Email Setup Wizard documentation, zero-OAuth-code OAuth positioning, the recommended stored-profile send flow, preferred diagnostics level functions vuSetDiagnosticsLevel() / vuGetDiagnosticsLevel(), the finalized build 157 logging model with diagnostics-off containment, expanded task help pages with real Clarion examples, restored old vuMail-compatible remote inline-image embedding for HTML body img src URLs, and clarified the easiest body-image path: Simple HTML with NormalizeBody on plus the embed-attachments marker for ordinary TEXT-control or memo content, including local image files and remote http/https image URLs passed through Attach. |
...
| 1.0 | 156 | 2026-06-22 | Restored legacy sent-mail CSV logging as a separate log from diagnostics. Added vuClearMailLog() for the sent-mail CSV log and vuClearDiagnosticsLog() as the preferred diagnostics clear function. |
