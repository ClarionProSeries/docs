...
if not exist "%OUT%\vuMailKit.hhp" (
  echo ERROR: vuMailKit.hhp not found in base folder.
  exit /b 1
)
...
if defined HHCEXE (
  pushd "%OUT%"
  "%HHCEXE%" "vuMailKit.hhp"
  popd
) else (
  echo hhc.exe not found. Open "%OUT%\vuMailKit.hhp" manually.
)
