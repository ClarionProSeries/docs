...
description: "Sets default From and Reply-To addresses in runtime globals. [All functions](index.md) | [Legacy functions](legacy-index.md) | [About function names](AboutFunctionNames.md)"
...
```Prototype
vuSmtpSetFromReplyTo(*CSTRING, *CSTRING), LONG
```
...
|---|---|---|
| FromAddr | *CSTRING | Sender address. Blank allowed; non-blank must be valid email format. |
| ReplyTo | *CSTRING | Reply-To address. Blank allowed; non-blank must be valid email format. |
...
MAP
  MODULE('vuMail.dll')
    vuSmtpSetFromReplyTo(*CSTRING,*CSTRING),LONG,PASCAL,RAW,NAME('vuSmtpSetFromReplyTo')
  END
END
