---
title: "vuSendMailWait"
summary: "Queues one email and waits for SMTP processing to finish."
description: "Queues one email and waits for SMTP processing to finish."
keywords: ["vuMailKit", "vuMail", "vusendmailwait", "smtp"]
function_name: "vuSendMailWait"
category: "SMTP"
...
- Attachments may be blank, a single full path, a comma/semicolon-separated list, or wildcard masks such as `C:\Temp\*.pdf`.
...
- The legacy `_embed attachments_` body tag is supported again for image attachments, including optional `height=` and `width=` attributes.
...
## Inline image example (Clarion source literal)
