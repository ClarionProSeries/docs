...
**Prototype:** vuPasswordInit(LONG InHwnd, LONG InInitialMode, *CSTRING InCueText, *CSTRING InToolTip, LONG InFlags), SIGNED, PROC, PASCAL, RAW, NAME('vuPasswordInit')
...
| InFlags | LONG | Controls whether the clipboard is cleared after a paste into the managed control. Pass 1 to clear the clipboard after paste. Pass 0 to leave the clipboard unchanged. |
...
## Example (Clarion): cue text, tooltip, and clipboard clearing enabled
...
RC = vuPasswordInit(?LoginName{PROP:Handle}, LoginNameShowState, LoginNameCue, LoginNameTip, |
  LoginNameClearClipboardAfterPaste)
```

## Example (Clarion): no cue text, no tooltip, and clipboard clearing disabled
...
RC = vuPasswordInit(?LoginPassword{PROP:Handle}, LoginPasswordShowState, LoginPasswordCue, |
  LoginPasswordTip, LoginPasswordClearClipboardAfterPaste)
```
