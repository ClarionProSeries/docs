...
for /r "%SRC%" %%F in (*.md) do (
  ...
  if defined SKIP (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File "%PS1%" "%%F" "!TMPDEST%"
    if errorlevel 1 (
      echo ERROR: strip_yaml.ps1 failed for "%%F"
      exit /b 1
    )

    echo pandoc "!TMPDEST!" -s -f gfm --lua-filter "%LUA%" --css "%CSS%" -o "!DEST!"
    pandoc "!TMPDEST!" -s -f gfm --lua-filter "%LUA%" --css "%CSS%" -o "!DEST!"
    if errorlevel 1 (
