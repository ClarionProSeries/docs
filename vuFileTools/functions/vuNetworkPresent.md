...
summary: "Detects whether a TCP/IP network is present and reports LAN and RAS or VPN style connectivity."
description: "Detects whether a TCP/IP network is present and reports LAN and RAS or VPN style connectivity. Returns 0 for no network, 1 when the LAN flag is set, 2 when the WAN (RAS or VPN) flag is set, and 3 when both flags are set. A WAN result does not guarantee Internet connectivity. For Internet checks, see vuInternetPresent and vuInternetCanReachUrl."
